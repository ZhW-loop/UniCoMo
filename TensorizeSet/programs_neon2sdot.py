...
def candidates_neon2sdot(workload_path, candidate_path, model_name,
    ...
    for tuning_record in tuning_records:
        ...
        assert "dot_4x4_i8i8s32_neon" in str(tuning_record)
        tuning_record = replace_substring_recursive(list(tuning_record), 
                                                    "dot_4x4_i8i8s32_neon", "dot_4x4_i8i8s32_sdot")
        assert "dot_4x4_i8i8s32_neon" not in str(tuning_record)
        assert "dot_4x4_i8i8s32_sdot" in str(tuning_record)
...
def main():
    ...
    for md in model_dirs:
        if 'neon' in str(md):
            neon_model_dirs.append(md)
    
    for model_dir in neon_model_dirs:
        model_name: str = model_dir.split("/")[-1]
        model_name = model_name.replace("neon", "sdot")
        ...
        for workload_path in tqdm(workload_paths):
            ...
            candidates_neon2sdot(workload_path, candidate_path, 
                               model_name, 
