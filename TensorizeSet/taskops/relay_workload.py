...
hw_cast_dtype = common.HW_CAST_DTYPE
...
def _get_network(
    ...
    if name in [
        ...
        params: Dict[str, Any] = {}
        if name in ["resnet_18", "resnet_50", "resnet_101"]:
        ...
        passes = [relay.transform.ToMixedPrecision(hw_cast_dtype), 
        ...
        inputs = (input_name, input_shape, hw_cast_dtype)
...
def _load_cache(cache_dir: Optional[str], filename: str) -> Optional[List[Any]]:
    ...
    if not os.path.exists(path):
        return None
    print("Loaded from cached: %s", path)
...
def get_network(
    ...
    *,
    ...
    filename = f'relay-{name}-{layout}-{",".join(str(i) for i in input_shape)}-{hw_cast_dtype}.json'
    ...
    if cached is None:
        ...
        mod, params_bytearray, inputs = _get_network((name, input_shape, layout))
        cached = [mod, params_bytearray, inputs]
        _save_cache(cache_dir, filename, cached)
...
def extract_from_relay(
    ...
    *,
    ...
    extracted_tasks : List[ExtractedTask]
        The extracted tasks.
    """
    filename = f'tasks-{name}-{",".join(str(i) for i in input_shape)}-{target.kind.name}-{hw_cast_dtype}.json'
